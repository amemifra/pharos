...
/**
 * Merge-preservation for the session queue record (#27 regression guard):
 * the shell-side queue/index write must NEVER clobber the island's fresher
 * position/playing (they are owned by the throttled pf.state save and only
 * refreshed there). Extracted pure so the DQ suite can pin the contract.
 * @param {{position?: number, playing?: boolean}|null} saved Existing record.
 * @param {{queue: Object[], index: number}} next Queue + index update.
 * @returns {Object} Merged record (spread keeps any future fields).
 */
export function mergeQueueRecord(saved, next) {
  return {
...
/**
 * Resume position for an episode (#25): maps the persisted listening
 * progress onto the episode duration honestly — never past the end, never a
 * fabricated number when nothing was recorded.
 * @param {{progressSec?: number}|null} prev pf.played:<guid> record.
 * @param {number|null} durationMs Episode duration (feed itunes:duration).
 * @returns {number} Seconds to seek to on play (0 = from the start).
 */
export function resumeSeconds(prev, durationMs) {
  const progress = Math.max(0, Math.round(Number(prev?.progressSec) || 0));
  if (!progress) return 0;
  const durationSec = Number(durationMs) > 0 ? Math.round(Number(durationMs) / 1000) : null;
  // Near the end (or past it): restart from the top rather than seek to EOF.
  if (durationSec !== null && progress >= durationSec * 0.95) return 0;
  return durationSec !== null ? Math.min(progress, Math.max(0, durationSec - 1)) : progress;
}
...
/**
 * Enqueue-time restricted guard (plan::40::gap-podcast-restricted-guard):
 * before ANY bulk enqueue (play-all, queue build) every episode's enclosure
 * is checked with lib/podcast.isPlayable (401/403/dead → restricted).
 * Restricted episodes NEVER enter the queue — the player-side pf.trackerror
 * auto-skip stays only as a safety net. Bounded work: concurrency 4, one
 * HEAD request per episode, original order preserved.
 * @param {Object[]} episodes Parsed feed episodes.
 * @param {(restricted: Object[]) => void} [onRestricted] Honest-UI callback.
 * @returns {Promise<Object[]>} Playable episodes only, original order.
 */
export async function filterPlayableEpisodes(episodes, onRestricted = null) {
  const eps = Array.isArray(episodes) ? episodes : [];
  if (!eps.length) return [];
  const { isPlayable } = await import("./podcast.js");
  const slots = new Array(eps.length).fill(false);
  let i = 0;
  const worker = async () => {
    while (i < eps.length) {
      const idx = i++;
      slots[idx] = await isPlayable(eps[idx]);
    }
  };
  await Promise.all(Array.from({ length: Math.min(4, eps.length) }, worker));
  const playable = [];
  const restricted = [];
  for (let k = 0; k < eps.length; k++) (slots[k] ? playable : restricted).push(eps[k]);
  if (restricted.length && onRestricted) { try { onRestricted(restricted); } catch {} }
  return playable;
}

 ...
 * Historical decision data for the WASM silence-detection upgrade: that port
 * has landed as lib/silence.js (mask kernel + JS fallback); this bench still
 * tells whether alignment on the median device needs the measured-boundary
 * path at all (<1 ms → proportional estimate is fine).
...
/**
 * Measures the silence-mask kernels (wasm vs JS fallback, lib/silence.js).
 * Adoption decision data for the measured-boundary path in compound-track
 * alignment: wasm is preferred only if t_js_p95 > t_wasm_p95 + t_load on the
 * target device (same criterion as benchTrackAlignment).
 * @param {Float32Array} pcm Mono PCM sample (e.g. 60 s at 44.1 kHz).
 * @param {number} [iterations=50]
 */
export async function benchSilenceDetection(pcm, iterations = 50) {
  const { silenceMask, silenceMaskRef } = await import("./silence.js");
  const run = async (label, fn) => {
    ...
    const s = stats(times);
    console.table([{ phase: label, ...s }]);
    return s;
  };
  const js = await run(`silenceMask JS n=${pcm.length}`, silenceMaskRef);
  const wasm = await run(`silenceMask wasm n=${pcm.length}`, silenceMask);
  return { js, wasm };
}

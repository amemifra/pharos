...
export function startWarmup({ batch = 4, gapMs = 30_000, onProgress } = {}) {
  ...
  (async () => {
    // 0) PD-FIRST (F1/F2): artists whose content is public domain by
    //    construction (curated collections) get filled before any popularity
    //    crawl — the queue starts where the law is already settled.
    try {
      const pd = await pdCandidates(100);
      for (const name of pd) {
        if (state.cancel) break;
        await idleGap(gapMs);
        if (state.cancel) break;
        const res = await fillArtist(name).catch(() => null);
        done++;
        onProgress?.({ artist: name, done, total: done + 1, fill: res?.state });
      }
    } catch { /* PD scan failing must not block the popularity warm-up */ }
    ...
    while (!state.cancel) {
      ...
      for (const { name } of ranked) {
        ...
        // F2: with the cross-reference in place, try the fill engine
        // (claim → extract → DQ → publish). All states are honest no-ops
        // when another peer holds the lease or the fill already exists.
        try {
          const res = await fillArtist(name);
          if (res.state === "filled") onProgress?.({ artist: name, done, total: done + 1, fill: "filled" });

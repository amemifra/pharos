...
// Lucene special characters — escape so user input can never inject field
// syntax or boolean operators into a query (400s or rewritten semantics).
// Applied ONLY at user-input interpolation points: internal queries that use
// real Lucene syntax (e.g. "-access-restricted-item:true") must stay untouched.
const LUCENE_SPECIAL = /([+\-!(){}\[\]^"~*?:\\/]|&&|\|\|)/g;
export function escapeLucene(s) {
  return String(s ?? "").replace(LUCENE_SPECIAL, (m) => `\\${m}`);
}
 ...
 */
export async function searchRanked({ query, collection, page = 1, rows = 24, rawQuery = null }) {
  // rawQuery (#14): pre-built Lucene syntax (genre tiles pass shelf.query
  // like "subject:(jazz)") — NEVER escaped, it is internal syntax, not user
  // input. escapeLucene stays on the user-typed path only.
  const [fielded, general] = await Promise.all([
    !rawQuery && query && !collection
      ? searchArchive({ query: `creator:("${escapeLucene(query)}")`, collection, page, rows })
      : Promise.resolve({ items: [], total: 0 }),
    searchArchive({ query: rawQuery ?? escapeLucene(query), collection, page, rows }),
  ]);
 ...
 * @param {number} [opts.page=1] Page number, 1-based.
 * @param {number} [opts.rows=24] Results per page.
 * @param {string} [opts.sort] archive.org sort, e.g. "downloads desc".
 * @returns {Promise<{items: Object[], total: number}>} Raw docs + total hit count.
 */
export async function searchArchive({ query, collection, page = 1, rows = 24, sort }) {
...
/**
 * Iterates the search pagination and returns ALL matching items (up to
 * maxItems): one `searchArchive` page is a WINDOW, not the result set — an
 * artist with numFound=126 at rows=100 was silently losing 26 items.
 * Contract: sequential pages (polite to archive.org); stops at `total`,
 * maxItems, or the first empty page; throws on HTTP errors like searchArchive.
 * @param {string} query
 * @param {{collection?: string, rows?: number, maxItems?: number, sort?: string}} [opts]
 * @returns {Promise<{items: Object[], total: number}>}
 */
export async function searchArchiveAll({ query, collection, rows = 100, maxItems = 1000, sort }) {
  const items = [];
  let total = 0;
  for (let page = 1; items.length < maxItems; page++) {
    const { items: docs, total: found } = await searchArchive({ query, collection, page, rows, sort });
    total = found;
...
/**
 * Search by artist (creator field), sorted by downloads — FULL result set:
 * iterates the pagination (searchArchiveAll) instead of stopping at the
 * first page window. Bounded at 1000 items (raw list is a secondary view;
 * crossref matching has its own source set).
...
export async function searchByArtist(artist) {
  try {
    const { items } = await searchArchiveAll({
    ...
    return []; // honest empty on error (same contract as before)
  }
}
...
/**
 * Classifies an item for the PD registry and the fill scheduler (F1).
 * Contract: pure, no network — the caller supplies the raw search doc (or the
 * /metadata payload fields). Only collections that have ALREADY made the
 * legal determination count as `curated`: the app never crowd-asserts PD.
 * @param {Object} doc Raw archive.org doc (fields: identifier, collection, licenseurl, license…).
 * @returns {{identifier: string, collections: string[], curated: boolean,
 *   pdLikely: boolean, license: string|null, reason: string}}
 */
export function classifyItem(doc) {
  const collections = Array.isArray(doc.collection) ? doc.collection : [doc.collection].filter(Boolean);
  // Curated = the collection's charter already resolved the rights question.
  const CURATED = new Set(["georgeblood", "netlabels", "etree"]);
  // georgeblood (78rpm discs) is public domain by construction.
  const PD_COLLECTIONS = new Set(["georgeblood"]);
  const license =
    typeof doc.licenseurl === "string" ? doc.licenseurl
    : typeof doc.license === "string" ? doc.license : null;
  const curated = collections.some((c) => CURATED.has(c));
  const pdCollection = collections.some((c) => PD_COLLECTIONS.has(c));
  const ccLicense = !!license && /creativecommons\.org\/(licenses|publicdomain)/.test(license);
  const reason = pdCollection ? "curated collection: PD by construction"
    : ccLicense ? "explicit CC/PD license field on the item"
    : curated ? "curated collection (rights-determined, not necessarily PD)"
    : "uncurated";
  return { identifier: doc.identifier, collections, curated, pdLikely: pdCollection || ccLicense, license, reason };
}

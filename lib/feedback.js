...
const ARTIST_LOCAL = "pf.artist.plays.local";
const ARTIST_PUBLISHED = "pf.artist.plays.published";
...
/**
 * Records that an artist was played (community listening chart).
 * Local counter first (`pf.artist.plays.local`), then merged additively into
 * the SHARED `listens:artists` table on OrbitDB by publishArtistListens()
 * with the same delta-only consensus as publishVersionConsensus: only
 * aggregate counts leave the device, never individual history.
 * Contract: pure side effect; never throws.
 * @param {string} artist Artist display name.
 * @param {{completed?: boolean}} [opts]
 * @returns {void}
 */
export function recordArtistListen(artist, { completed = false } = {}) {
  const name = String(artist ?? "").trim();
  if (!name) return;
  try {
    const table = JSON.parse(localStorage.getItem(ARTIST_LOCAL) ?? "{}");
    const cell = (table[name] ??= { plays: 0, completes: 0 });
    cell.plays += 1;
    if (completed) cell.completes += 1;
    localStorage.setItem(ARTIST_LOCAL, JSON.stringify(table));
  } catch {}
}

/**
 * Publishes local artist-play aggregates to the shared listening chart (P2P).
 * Delta only: counters already published are never re-counted (same rule as
 * publishVersionConsensus). Contract: never throws; true when published.
 * @returns {Promise<boolean>}
 */
export async function publishArtistListens() {
  try {
    const local = JSON.parse(localStorage.getItem(ARTIST_LOCAL) ?? "{}");
    if (!Object.keys(local).length) return false;
    let published = {};
    try { published = JSON.parse(localStorage.getItem(ARTIST_PUBLISHED) ?? "{}"); } catch {}
    const store = await import("./catalogstore.js");
    const key = store.keys.artistListens();
    const shared = (await store.catalogGet(key)) ?? {};
    const merged = { ...shared };
    let dirty = false;
    for (const [name, cell] of Object.entries(local)) {
      const already = published[name] ?? { plays: 0, completes: 0 };
      const dPlays = (cell.plays ?? 0) - (already.plays ?? 0);
      const dComp = (cell.completes ?? 0) - (already.completes ?? 0);
      if (dPlays > 0 || dComp > 0) {
        const prev = merged[name] ?? { plays: 0, completes: 0 };
        merged[name] = { plays: prev.plays + Math.max(0, dPlays), completes: prev.completes + Math.max(0, dComp) };
        dirty = true;
      }
    }
    if (!dirty) return false;
    // Footprint cap: keep the 500 most-played artists.
    const entries = Object.entries(merged)
      .sort((a, b) => (b[1].plays ?? 0) - (a[1].plays ?? 0))
      .slice(0, 500);
    await store.catalogSet(key, Object.fromEntries(entries));
    try { localStorage.setItem(ARTIST_PUBLISHED, JSON.stringify(local)); } catch {}
    return true;
  } catch {
    return false;
  }
...
/**
 * Community listening chart: the artists PharOS users actually play, from the
 * SHARED `listens:artists` table (OrbitDB + localStorage cache), most plays
 * first. This — not Wikipedia pageviews — is what "popular" means in PharOS.
 * Contract: never throws; [] before any listen is shared/synced.
 * @param {number} [limit] Max entries (default 24).
 * @returns {Promise<{name: string, plays: number, completes: number}[]>}
 */
export async function topListenedArtists(limit = 24) {
  try {
    const store = await import("./catalogstore.js");
    const shared = (await store.catalogGet(store.keys.artistListens())) ?? {};
    return Object.entries(shared)
      .map(([name, c]) => ({ name, plays: c?.plays ?? 0, completes: c?.completes ?? 0 }))
      .filter((a) => a.plays > 0)
      .sort((a, b) => b.plays - a.plays || b.completes - a.completes)
      .slice(0, limit);
  } catch {
    return [];
  }

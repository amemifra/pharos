...
import { searchArchiveAll } from "./archive.js";
...
export function crossReference(artistName, { sourceCollection = SOURCE_COLLECTION_QUERY } = {}) {
...
async function crossReferenceCompute(artistName, { sourceCollection = SOURCE_COLLECTION_QUERY } = {}) {
  ...
  const [canonical, sourceItems] = await Promise.all([
    ...
    searchArchiveAll({ query: `creator:"${artistName}"`, collection: sourceCollection, rows: 100, maxItems: 1000, sort: "downloads desc" }).then((r) => r.items.map(normalizeSearchItem)),
  ]);

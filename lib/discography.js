 ...
 */
export async function resolveArtistMbid(name) {
  const headers = typeof window === "undefined" ? { "User-Agent": "Pharos/0.1 (catalog crossref)" } : undefined;
  // Interactive: artist resolution must jump ahead of warm-up traffic.
  const res = await mbRequest(`https://musicbrainz.org/ws/2/artist?query=${encodeURIComponent(`artist:"${name}"`)}&fmt=json&limit=5`, { priority: "user" });
 ...
 * @param {Object} [opts]
 * @param {string} [opts.sourceCollection="audio_music"] archive.org collection to search into.
 * @returns {Promise<{artist: string, entries: Array, available: number, missing: number, coverage: number}>}
 */
export function crossReference(artistName, { sourceCollection = "audio_music" } = {}) {
  return catalogMemo(keys.crossref(artistName), () => crossReferenceCompute(artistName, { sourceCollection }));
}

async function crossReferenceCompute(artistName, { sourceCollection = "audio_music" } = {}) {
  const resolved = await resolveArtistMbid(artistName);
  if (!resolved) {
  ...
  const [canonical, sourceItems] = await Promise.all([
    ...
    searchArchive({ query: `creator:"${artistName}"`, collection: sourceCollection, rows: 100 }).then((r) => r.items.map(normalizeSearchItem)),
  ]);

  // For each canonical entry: best available match by trigram + year window.
  const entries = canonical.map((rg) => {
    let best = null;
    let bestScore = 0;
    for (const item of sourceItems) {
      const s = trigram(rg.title, item.title);
      const yearOk = rg.year == null || item.year == null || Math.abs(rg.year - item.year) <= 2;
    ...
    return bestScore >= MATCH_THRESHOLD && best
      ? { ...rg, status: "available", match: { id: best.id, title: best.title, year: best.year, score: bestScore } }
      : { ...rg, status: "missing" };
  });

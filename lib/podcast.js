...
/**
 * Fetches feed XML: direct first, declared CORS-proxy fallback.
...
export function parseFeed(xml, feedUrl, cap = 500) {
  ...
  const show = {
    ...
    image: ch.querySelector("image > url")?.textContent?.trim()
      || (ch.querySelector("itunes\\:image")?.getAttribute("href") ?? null),
    link: get("link"),
    at: Date.now(),
  };

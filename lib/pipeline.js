...
/**
 * Alternate-take folding (DQ stage): a file titled "<Work> (Take 1)" or
 * "<Work> (Mono Mix RM 11)" is NOT a distinct track — it is an alternate
 * version of the work already listed. Publishing it as a separate numbered
 * track is exactly the "repeated tracks" DQ failure observed on items like
 * the Revolver 2022 mix (canonical 1-14 + bonus takes rendered as 1-18).
 *
 * Rule (objective): a trailing parenthesized/bracketed qualifier that
 * declares a take/mix/version/rehearsal/work-tape AND whose base title
 * matches an already-kept track → folded into that track as alternates[]
 * (still playable, never invented: the file keeps its own id/url/format/
 * duration).
 */
const TRAILING_QUALIFIER = /\s*[\(\[]([^\)\]]{2,60})[\)\]]\s*$/;
// "take"|"mix"|… cover the observed archive.org spellings (Revolver 2022
// deluxe: Take N, Mono, Mono Mix RM1, First/Second Version, John's Demo,
// Unnumbered Rehearsal, Songwriting Work Tape, Highlighted Sound Effects).
const ALTERNATE_Q = /take|mix|version|remaster|remix|mono|stereo|rm\s*\d|session|alternate|demo|outtake|early|first|second|acoustic|instrumental|rehearsal|work\s*tape|sound\s*effects|backing/i;
// Qualifier made only of mix-words ("( Mix)", leftover after stageCleanTitle
// strips "(2022 Mix)" → "( Mix)", or "( Stereo Mix)", the default mix): noise
// for display, drop it. Meaningful qualifiers never match: "Mono" (mono
// version), "Take 11", "Mono Mix RM1" — digits and standalone mono are
// outside the class, leading spaces are consumed by the head glue.
const NOISE_QUALIFIER = /^[\s\-–—.]*(?:(?:stereo\s+)?(?:remaster|remix|remastered|version|mix)\b[\s\-–—.]*)+$/i;

function splitQualifier(title) {
  const m = String(title).match(TRAILING_QUALIFIER);
  return m
    ? { base: title.slice(0, m.index).trim(), qualifier: m[1] }
    : { base: String(title).trim(), qualifier: null };
}

/** Drops a trailing qualifier that carries no information ("( Mix)"). */
function stripNoiseQualifier(title) {
  const { base, qualifier } = splitQualifier(title);
  return qualifier && NOISE_QUALIFIER.test(qualifier) ? base : String(title).trim();
}

/** Stable grouping key of a track title: base title, case/punct-insensitive. */
const normBaseKey = (s) => String(s).toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
export const trackBaseKey = (title) => normBaseKey(splitQualifier(title).base);
...
function trackTitle(file, index) {
  ...
  const t = stripNoiseQualifier(doc.cleanTitle);
 ...
 */
/**
 * Folds alternate takes/mixes into their base track.
 * Contract: pure; returns a NEW array; alternates keep full play metadata.
 * @param {Object[]} tracks Normalized tracks (rawTitle preferred over title).
 * @returns {Object[]} Tracks with alternates folded in.
 */
export function foldAlternateTakes(tracks) {
  const kept = [];
  const byBase = new Map();
  for (const t of tracks) {
    const { base, qualifier } = splitQualifier(t.rawTitle ?? t.title);
    const key = normBaseKey(base);
    const host = byBase.get(key);
    if (qualifier && ALTERNATE_Q.test(qualifier) && host) {
      host.alternates = [
        ...(host.alternates ?? []),
        { id: t.id, title: t.title, url: t.url, format: t.format, duration: t.duration, variants: t.variants ?? [] },
      ];
      continue; // not a separate numbered track
    }
    kept.push(t);
    if (!byBase.has(key)) byBase.set(key, t);
  }
  return kept;
}
...
export function normalizeAlbum(identifier, meta) {
  ...
  // Alternate-take folding (DQ): "<Work> (Take 1)" is not a separate track.
  // Must run BEFORE numbering so display numbers stay canonical 1..n.
  const folded = foldAlternateTakes(tracks);

  // Sequential display numbering: see normalizeAlbum contract above.
  folded.forEach((t, i) => { t.number = i + 1; });

...
/**
 * CULTURAL CANON hook (lib/culture.js): per-culture tier boosts on top of the
 * global CANON. Stored here (not in culture.js) so every canon merge — boot,
 * community import — keeps the cultural layer without circular imports.
 * @type {Object}
 */
let culturalCanon = {};

/**
 * Sets the cultural boost table (called at boot by applyCulturalCanon).
 * @param {Object} table canonKey → extra tier.
 * @returns {void}
 */
export function setCultureCanon(table) {
  culturalCanon = table ?? {};
  setActiveCanon({ ...CANON, ...culturalCanon });
}
...
export async function importCanon(db) {
  // NON-WIRED (#30, declared deferred removal — not silent): no caller passes
  // an open OrbitDB handle today (the shell never boots collab for this), so
  // only the builtin/localCache tiers are reachable. Kept as the documented
  // seam; remove alongside shareMetrics if still unused at next review.
  let table = null;
  let source = "builtin";
  ...
  if (table) setActiveCanon({ ...CANON, ...culturalCanon, ...table });

 ...
 *  soon because absence of evidence is not evidence of absence. 24h (was 1h):
 *  the long tail is mostly 0-available — a 1h ceiling made every reload
 *  re-process the same artists, defeating the "one computation, shared" rule.
 *  Once a day is still honest (sources grow) without being a treadmill. */
export const UNVERIFIED_TTL_MS = 24 * 3600 * 1000;
...
let dbPromise = null;
/** Logged-once flag: getDb failures are diagnosed honestly, but once —
 *  an offline user must not spam the console on every catalog read. */
let dbFailureLogged = false;
/** Hard ceiling for P2P reads/writes: a slow or wedged Helia/OrbitDB never
 *  blocks the UI — the local cache answers. (Mobile New Episodes fix: the
 *  section hung forever because catalogGet awaited a failing/hung node.) */
const P2P_TIMEOUT_MS = 2_500;

/** Races a promise against the P2P ceiling: null on timeout, never hangs. */
function bounded(promise, ms = P2P_TIMEOUT_MS) {
  ...
  if (!dbPromise) {
    dbPromise = (async () => {
      try {
        ...
        // DEFECT FIX (honest diagnosis): failures were silently swallowed —
        // getDb() returned null with NO trace, so P2P outages were invisible
        // (the mobile New Episodes bug surfaced exactly like this). Log ONCE.
        if (!dbFailureLogged) {
          dbFailureLogged = true;
          console.warn("[catalog] P2P layer unavailable — local cache only:", String(err?.message ?? err).slice(0, 200));
        }
        return null; // offline / unsupported: local cache only
      }
    })();
    // Watchdog: a node that never finishes starting must not tax every later
    // catalog read with the full P2P ceiling — swap the dead promise for an
    // instant null (a clean stopCollab/restart would build a fresh handle).
    setTimeout(() => {
      Promise.race([dbPromise, new Promise((r) => setTimeout(r, 100))]).then((v) => {
        if (v === undefined) dbPromise = Promise.resolve(null);
      });
    }, 10_000);
  }
  return dbPromise;
...
export async function catalogGet(key) {
  // Bounded P2P read: a wedged node answers null after the ceiling and the
  // local cache (fresh from the last successful sync) serves the caller.
  const db = await bounded(getDb());
  if (db) {
    try {
      const remote = await bounded(db.get(key));
      if (remote != null) {
        localSet(key, remote);
        return remote;
      }
...
export async function catalogSet(key, value) {
  ...
  if (!db) return false;
  try {
    const ok = await bounded(db.put(key, value));
    return ok != null;
  } catch { return false; }
}
